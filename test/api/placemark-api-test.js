...
import { maggie, maggieCredentials, ucc, testPlacemarks } from "../fixtures.js";
...
suite("Placemark API tests", () => {
  ...
  test("create placemark", async () => {
    const returnedPlacemark = await placemarkService.createPlacemark(ucc);
    assert.isNotNull(returnedPlacemark);
    assertSubset(ucc, returnedPlacemark);
  });

  test("delete a placemark", async () => {
    const placemark = await placemarkService.createPlacemark(ucc);
